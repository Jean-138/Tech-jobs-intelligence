...
def grafico_paises(df):  
    paises = df["location"].str.split(", ").explode()  # separa os países e transforma em várias linhas
    ...
    plt.title("Top 10 Countries with Most Jobs")  # define o título do gráfico
    plt.xlabel("Country")  # define o rótulo do eixo X
    plt.ylabel("Number of Jobs")  # define o rótulo do eixo Y
...
def grafico_contratos(df):
    contagem = df["type"].value_counts() #Pega a coluna "tipo" e Conta quantas vezes cada tipo aparece
    ...
    plt.title("Contract Types") #define o titulo do grafico
    plt.ylabel("")
    plt.tight_layout() # ajusta o layout automaticamente
...
def grafico_cargos(df): 
    contagem = df["title"].value_counts().head(10)  #quantas vezes cada cargo aparece e seleciona os 10 mais frequentes
    
    plt.figure(figsize=(12, 6))  #  cria a figura com tamanho 12x6
    contagem.plot(kind="barh", color="green")  # gráfico de barras horizontal com as contagens verde
    plt.title("Top 10 Job Titles with Most Openings")  # adiciona o título do gráfico
    plt.xlabel("Number of Jobs")  # rótulo do eixo X mostrando o número de vagas
    plt.ylabel("Job Title")  # define o rótulo do eixo Y mostrando o nome do cargo
    ...
    print("Chart saved: top job titles!")  # imprime mensagem confirmando que o gráfico foi salvo
...
def grafico_salarios(df):
    contagem = df["salary"].apply(  # Aplica uma transformação na coluna 'salario' do DataFrame
        lambda x: "Specified" if x != "unspecified" else "Not Specified" # Se o valor do salário não for "unspecified", marca como "Especificado", caso contrário marca como "Não Especificado"
    )
    ...
    plt.title("Jobs with Specified Salary")  # Define o título do gráfico
    ...
    print("Salary chart saved!")  # Exibe uma mensagem de confirmação de que o gráfico foi salvo
